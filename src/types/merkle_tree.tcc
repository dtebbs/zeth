    ...
    depth(depth)
{
    ...
    {
        last = get_hash<FieldT>({last}, last, get_iv_mt());
    ...
    merkle_tree<FieldT, HashT>(depth)
{
    ...
    for (int layer = depth; layer > 0; --layer)
    {
        for (size_t idx = idx_begin; idx < idx_end; idx += 2)
        {
            ...
            FieldT h = get_hash<FieldT>({l}, r, get_iv_mt());
            hashes[(idx-1)/2] = h;
        }
        ...
        for (int layer = depth; layer > 0; --layer)
        {
            ...
            for (auto it = hashes.begin(); it != last_it; ++it)
            {
                ...
                if (idx % 2 == 0)
                {
                    ...
                    hashes[(idx-1)/2] = get_hash<FieldT>({hash_defaults[layer]}, hash, get_iv_mt());
                }
                else
                {
                    if (std::next(it) == last_it || std::next(it)->first != idx + 1)
                    {
                        ...
                        hashes[(idx-1)/2] = get_hash<FieldT>({hash}, hash_defaults[layer], get_iv_mt());
                    }
                    else
                    {
                        // typical case: this is the left child of the parent and adjacent to it there is a right child
                        hashes[(idx-1)/2] = get_hash<FieldT>({hash}, std::next(it)->second, get_iv_mt());
                        ++it;
                    }
                }
            }
