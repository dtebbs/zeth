...
private:
  libzeth::CircuitWrapper<FieldT, HashT, ZETH_NUM_JS_INPUTS, ZETH_NUM_JS_OUTPUTS> prover;
...
public:
  ...
  Status Prove(
    ...
    try {
      FieldT root = string_to_field<FieldT>(proofInputs->root());
      FieldT vpub_in = string_to_field<FieldT>(proofInputs->inpubvalue());
      FieldT vpub_out = string_to_field<FieldT>(proofInputs->outpubvalue());
      ...
      std::array<libzeth::ZethNote<FieldT>, ZETH_NUM_JS_OUTPUTS> jsOutputs;
      for(int i = 0; i < ZETH_NUM_JS_OUTPUTS; i++) {
        proverpkg::ZethNote receivedOutput = proofInputs->jsoutputs(i);
        libzeth::ZethNote<FieldT> parsedOutput = ParseZethNote<FieldT>(receivedOutput);
